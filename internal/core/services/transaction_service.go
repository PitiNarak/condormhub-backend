...
func (s *TransactionService) CreateTransaction(orderID uuid.UUID) (*domain.Transaction, *string, *error_handler.ErrorHandler) {
	...
	if err != sErr {
		return nil, nil, error_handler.InternalServerError(err, "Failed to create payment session")
	}
...
func (s *TransactionService) UpdateTransactionStatus(event stripe.Event) *error_handler.ErrorHandler {
