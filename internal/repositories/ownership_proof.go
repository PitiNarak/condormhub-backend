...
func NewOwnershipProofRepository(db *gorm.DB) ports.OwnershipProofRepository {
...
func (o *OwnershipProofRepository) Create(ownershipProof *domain.OwnershipProof) error {
	if err := o.db.Create(ownershipProof).Error; err != nil {
		...
		return errorHandler.InternalServerError(err, "Failed to save ownership proof to database")
	}
	return nil
}
...
func (o *OwnershipProofRepository) Delete(lessorID uuid.UUID) error {
	if err := o.db.Delete(&domain.OwnershipProof{}, lessorID).Error; err != nil {
		return errorHandler.InternalServerError(err, "Failed to delete ownership proof")
	}
	return nil
}
...
func (o *OwnershipProofRepository) GetByLessorID(lessorID uuid.UUID) (*domain.OwnershipProof, error) {
	...
	if err := o.db.First(ownershipProof, lessorID).Error; err != nil {
		return nil, errorHandler.NotFoundError(err, "Ownership proof not found")
	}
...
func (o *OwnershipProofRepository) UpdateDocument(lessorID uuid.UUID, updateDocumentRequestBody *dto.UpdateOwnerShipProofRequestBody) error {
	...
	if err != nil {
		return errorHandler.NotFoundError(err, "Ownership proof not found")
	}
	updateErr := o.db.Model(existingOwnershipProof).Where("lessorID = ?", lessorID).Updates(updateDocumentRequestBody).Error
	if updateErr != nil {
		return errorHandler.InternalServerError(err, "failed to update document")
	}
...
func (o *OwnershipProofRepository) UpdateStatus(lessorID uuid.UUID, updateStatusRequestBody *dto.UpdateOwnerShipProofStatusRequestBody) error {
	...
	if updateErr != nil {
		return errorHandler.InternalServerError(err, "failed to update status")
	}
